    ...
    def getMoves(self, board):
        ...
        for i in range(1, 8):
            if self.position[0] + i < 8:
                if board[self.position[0] + i][self.position[1]] == 0:
                    moves.append((self.position[0] + i, self.position[1]))
                else:
                    if board[self.position[0] + i][self.position[1]].getColor() != self.color:
                        moves.append((self.position[0] + i, self.position[1]))
                    break
            else:
                break
        ...
        for i in range(1, 8):
            if self.position[1] + i < 8:
                if board[self.position[0]][self.position[1] + i] == 0:
                    moves.append((self.position[0], self.position[1] + i))
                else:
                    if board[self.position[0]][self.position[1] + i].getColor() != self.color:
                        moves.append((self.position[0], self.position[1] + i))
                    break
            else:
                break
